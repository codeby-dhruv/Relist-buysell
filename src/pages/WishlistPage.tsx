    ...
    <div className="space-y-5 px-4 pt-4 md:px-0 md:pt-0">
      <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-slate-950 md:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-normal text-slate-400">Saved collection</p>
            <h1 className="inter-copy mt-2 text-3xl font-semibold">Saved products</h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
              Keep your shortlisted listings organized so you can compare price, seller and location quickly.
            </p>
          </div>
          <div className="w-fit rounded-2xl bg-slate-50 px-4 py-3 dark:bg-white/5">
            ...
            <p className="text-xs font-normal text-slate-500">saved items</p>
          </div>
        </div>
      </section>
    ...
    <article className="grid grid-cols-[92px_1fr] gap-3 p-4 transition hover:bg-slate-50 dark:hover:bg-white/5 md:grid-cols-[112px_1fr_auto] md:items-center md:p-5">
      ...
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          {product.isFeatured && <span className="rounded-full bg-teal-500/10 px-2.5 py-1 text-[11px] font-normal text-teal-700 dark:text-teal-300">Featured</span>}
